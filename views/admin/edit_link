    ...
    <form method="POST" action="/admin/edit-link?code={{ urlencode($link->getCode()) }}" class="edit-link-form"
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <input type="number" id="wait_seconds" name="wait_seconds" value="{{ $link->getWaitSeconds() }}" min="0"
            max="60">
        ...
        <div class="form-group">
          ...
          <div class="image-upload-container">
            ...
            <div class="image-upload-preview" id="image-preview">
              ...
              <img src="{{ $link->getAdsImgUrl() }}" style="max-width: 100%; max-height: 200px; border-radius: 4px;">
              @else
              <span class="upload-text">Click to upload image or drag & drop</span>
              @endif
            </div>
          </div>
          <input type="hidden" id="ads_img_url" name="ads_img_url" value="{{ $link->getAdsImgUrl() ?? '' }}">
          <small>Upload an image for advertisement display</small>
        </div>

        <div class="form-group">
          <label for="ads_click_url">Ad Click URL</label>
          <input type="url" id="ads_click_url" name="ads_click_url" value="{{ $link->getAdsClickUrl() ?? '' }}">
          <small>URL to redirect to when ad is clicked</small>
        </div>

        <div class="form-group">
          <label for="ads_promoted_by">Promoted By</label>
          <input type="text" id="ads_promoted_by" name="ads_promoted_by" value="{{ $link->getAdsPromotedBy() ?? '' }}">
