    ...
    <div class="map-analytics-container">
        ...
        <div class="map-stats">
            ...
            <div class="stat-card">
                ...
                <span class="stat-number" id="top-location">' . (!empty($locationData) ? $locationData[0]['location'] : 'N/A') . '</span>
            </div>
        </div>
...
foreach ($locationData as $location) {
  ...
  foreach (array_slice($location['ip_addresses'], 0, 3) as $ip) {
    $content .= '<span class="ip-tag">' . htmlspecialchars($ip) . '</span>';
  }

  if (count($location['ip_addresses']) > 3) {
    $content .= '<span class="ip-more">+' . (count($location['ip_addresses']) - 3) . ' more</span>';
  }
