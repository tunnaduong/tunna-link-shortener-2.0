...
@section('content')
    <div class="create-link-page">
                        ...
                        <div class="form-group">
                            ...
                            <div class="url-input-group">
                                <input type="url" id="next_url" name="next_url" required
                                    placeholder="https://example.com" value="{{ $_POST['next_url'] ?? '' }}">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="link_title" name="link_title" placeholder="My Awesome Link"
                                value="{{ $_POST['link_title'] ?? '' }}">
                        </div>

                        <div class="form-group">
                            <label for="link_excerpt">Description (Optional)</label>
                            <textarea id="link_excerpt" name="link_excerpt" rows="3" placeholder="Brief description of the link">{{ $_POST['link_excerpt'] ?? '' }}</textarea>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" id="ads_click_url" name="ads_click_url"
                                placeholder="https://example.com/ads" value="{{ $_POST['ads_click_url'] ?? '' }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="ads_promoted_by" name="ads_promoted_by"
                                placeholder="Company Name or Brand" value="{{ $_POST['ads_promoted_by'] ?? '' }}">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <select id="redirect_type" name="redirect_type">
                                <option value="0" {{ ($_POST['redirect_type'] ?? '0') == '0' ? 'selected' : '' }}>
                                    Direct Redirect</option>
                                <option value="1" {{ ($_POST['redirect_type'] ?? '0') == '1' ? 'selected' : '' }}>
                                    Click Through</option>
                                <option value="2" {{ ($_POST['redirect_type'] ?? '0') == '2' ? 'selected' : '' }}>
                                    reCAPTCHA Protected</option>
                                <option value="3" {{ ($_POST['redirect_type'] ?? '0') == '3' ? 'selected' : '' }}>
                                    Password Protected</option>
                                <option value="4" {{ ($_POST['redirect_type'] ?? '0') == '4' ? 'selected' : '' }}>
                                    Instant Redirect (No Ads)</option>
                            </select>
                        </div>
        ...
        <div id="batch-tab" class="tab-content">
            <div class="batch-shorten-container">
                ...
                <p>Enter URLs to shorten, one per line. You can use simple URLs or the advanced format with pipe separators.
                </p>
                ...
                <form id="batch-form" method="POST" action="/admin/batch-shorten">
                    ...
                    <div class="form-group">
                        ...
                        <select id="default_redirect_type" name="default_redirect_type">
                            ...
                            <option value="4">Instant Redirect (No Ads)</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="default_wait_seconds">Default Wait Time (seconds)</label>
                        <input type="number" id="default_wait_seconds" name="default_wait_seconds" min="0"
                            max="60" value="10">
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <div class="image-upload-container">
                                <input type="file" id="batch_ads_image_file" name="batch_ads_image_file"
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" id="batch_ads_click_url" name="batch_ads_click_url"
    ...
    <script>
        ...
        function clearBatchForm() {
            ...
            document.getElementById('batch-image-preview').innerHTML =
                '<span class="upload-text">Click to upload image or drag & drop</span>';
        }
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            batchAdsImageUploadInput.addEventListener("change", function(e) {
                ...
                if (file) {
                    const reader = new FileReader();
                    reader.onload = function(e) {
                        batchAdsImagePreview.innerHTML =
                            `<img src="${e.target.result}" style="max-width: 100%; max-height: 200px; border-radius: 4px;">`;
            ...
            function showExtractedTags(data) {
                ...
                headerRow.innerHTML =
                    "<th style=\"padding: 8px; text-align: left; border: 1px solid #dee2e6;\">Tag</th><th style=\"padding: 8px; text-align: left; border: 1px solid #dee2e6;\">Value</th>";
                ...
                Object.keys(data).forEach(key => {
                    if (data[key] && key !== "url") {
                        ...
                        tagCell.style.cssText =
                            "padding: 8px; font-weight: 500; color: #6c757d; border: 1px solid #dee2e6;";
                        tagCell.textContent = key;

                        const valueCell = document.createElement("td");
                        valueCell.style.cssText =
                            "padding: 8px; border: 1px solid #dee2e6; word-break: break-all;";

                        // Handle image tags specially
                        if (key === "image" || key === "og:image") {
                            valueCell.innerHTML =
                                `<img src="${data[key]}" style="max-width: 200px; max-height: 100px; border-radius: 4px; display: block; margin-top: 5px;">`;
                        } else {
